...
def visualization():
    ...
    def vis_step():
        ...
        previous_zones_distribution: dict[str, float] = {}
        for zone_id, zone in model.moving.zones.items():
            previous_zones_distribution[zone_id] = zone['NumPeople']
        model.step()

        # restore previous number of people in zones
        for zone_id in model.moving.zones:
            for prev_zone_id, num_of_people in previous_zones_distribution.items():
    ...
    def request_distribution_data():
        try:
            response = requests.get(
                f'{SERVER_URL}/distribution',
                timeout=DEFAULT_REQUEST_TIMEOUT_IN_SEC
            )
            rooms_distribution: dict[str, float] = json.loads(response.content)
            logging.info(rooms_distribution)
            for zone_id in model.moving.zones:
            ...
            logging.error('Cannot get distribution. Server is not responding')
